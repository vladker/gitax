...
class NuGetArchiver(LogMixin, BrowserInitMixin):
    ...
    def load_runtime(self):
        """Загрузить .NET runtime installer (первичная загрузка)."""
        from datetime import datetime
        ...
        runtime = RuntimeFactory.get_runtime("nuget")
        print(f"\n  {RuntimeFactory.get_icon('dotnet')} Загрузка .NET runtime...")

        latest = runtime.get_latest_version()
        if not latest:
            print("  ⚠ Не удалось получить версию .NET. Пропуск.")
            return

        print(f"  📦 .NET {latest} — загрузка инсталляторов для всех ОС...")
        ...
        for url_info in urls:
            ...
            if file_path and os.path.exists(file_path):
                ...
                sent = self._send_file_to_channel(
                    file_path,
                    message=f".NET {latest} — {os_name} installer\n\n{RuntimeFactory.get_download_page('dotnet')}",
                    browser=browser,
                )
                if sent:
                    entries.append({
        ...
        if entries:
            ...
            print(f"\n  ✓ .NET runtime {latest} загружен в журнал")
        else:
    ...
    def sync_runtimes(self):
        """Check and sync .NET runtime installer if a newer version is available."""
        from datetime import datetime
        from runtime_api import RuntimeFactory, OSTarget
        ...
        runtime = RuntimeFactory.get_runtime("nuget")
        print(f"\n  {RuntimeFactory.get_icon('dotnet')} Проверяю .NET runtime...")
        ...
        if not self.journal.should_update_runtime(latest):
            ...
            print(f"  ✓ .NET {saved} — актуален")
            return

        print(f"  🆕 .NET {latest} доступен (текущий: {self.journal.get_runtime_version() or 'не установлен'})")
        ...
        if entries:
            self.journal.set_runtime_version(latest, entries)
            print(f"\n  ✓ .NET runtime {latest} обновлён в журнале")
        else:
